...
export const getDocuments = async (email: string) => {
    try {
        ...
        console.log(`Error while getting the rooms: ${error}`);
    }
}

export const updateDocumentAccess = async ({ roomId, email, userType, 
    updatedBy }: ShareDocumentParams) => {
        try {
            ...
            console.log(`Error while updating the room access: ${error}`);
            
        }
    }

    export const removeCollaborator = async ({ roomId, email}: {roomId: string; email: 
        string}) => {
            try {
                const room = await liveblocks.getRoom(roomId);
